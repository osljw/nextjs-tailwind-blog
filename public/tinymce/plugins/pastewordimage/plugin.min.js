...
;(function () {
  ...
  function extractImageDataFromRtf(rtfData) {
    ...
    // ✅ 更通用的 RTF 图片正则（匹配 pngblip/jpegblip 开头 + 十六进制图像数据）
    const regexPicture =
      /{\\pict[\s\S]*?(pngblip|jpegblip)[\s\S]*?}(?:\\\*\\blipuid\s[\da-fA-F]+)?\s*([a-fA-F0-9\s\r\n]+?)\s*}}+/g
    ...
    try {
      while ((match = regexPicture.exec(rtfData)) !== null) {
        ...
        // 🔧 清理十六进制数据中的空格或换行
        const hexClean = hexRaw.replace(/[^a-fA-F0-9]/g, '')
        console.log('hexClean', hexRaw)

        const base64 = `data:${imageType};base64,${hexToBase64(hexClean)}`
        result.push({
          type: imageType,
          base64,
  ...
  const getImgFiles = (e) => {
    if (e.clipboardData || e.originalEvent) {
      ...
      if (clipboardData.items) {
        ...
        for (var i = 0; i < len; i++) {
          // console.log('items[i].type:', i, items[i].type, clipboardData.getData(items[i].type))
